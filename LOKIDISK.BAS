...
disk=disk+padfile(bootfile,256)
disk=disk+padfile(bdosfile,2816)
disk=disk+padfile(biosfile,1024)

print "Disk Boot Image Created - LOKI.DSK ";len(disk);" bytes"



nextalloc=2						: rem Start with allocation 2 - Allocations 0 and 1 are used for the directory. 
...
directory=pade5(directory,2048)	: rem size of 2 directory allocations.
directory=left(directory,2048)
