...
.equ MYBDOS,$F000
.equ DRCCP,$E500
.equ BIOSJUMP,$FC3E		; BIOS Jump Vector for warm boot. 
...
CCP_FILENAME:		DB	'ALTCCP  BIN'														; CCP Filename - and it's drive L:	
...
CCP_FILENAME_HND:	DB $00,$00,$00,$00	
...
LD	HL,$8000
LD	DE,DRCCP
LD	BC,$0806 ; half a block.  Plus 6 for the serial. 
LDIR		; transfer it AS A BLOCK. 

LD		HL,DRCCP+$03	; clean boot. 
;LD		($FC04),HL	; write new CCP location into BIOS WBOOT jump. 
LD		(BIOSJUMP),HL	; write new CCP location into BIOS WBOOT jump vector. 
...
M1: DB 'Preparing to migrate CCP to E500 and install hook.',$0D,$0A,'$'

M2: DB 'CCP Hook installed...',$0D,$0A,'$'
...
CCPPRELOAD:			DB	'Loading ALTCCP.BIN to buffer',$0A,$0D,'$'		
CCPERROR:			DB	'ALTBDOS.BIN not found. Exiting',$0A,$0D,'$'

BDOSOVER:		DB	'Error: Attempted to overwrite existing CCP - Exiting.',$0D,$0A,'$'
BDOSSUCCESS:	DB	'CCP Successfully loaded at E500',$0D,$0A,'$'
