...
;
EQU		MEM	,	62		;for a 62k system (TS802 TEST - WORKS OK).
;
EQU		IOBYTE	,	3		;i/o definition byte.
EQU		TDRIVE	,	4		;current drive name and user number.
EQU		ENTRY	,	5		;entry point for the cp/m bdos.
EQU		TFCB	,	$5C		;default file control block.
EQU		TBUFF	,	$80		;i/o buffer and command line storage.
EQU		TBASE	,	$100	;transiant program storage area.
;
;   Set control character equates.
;
EQU		CNTRLC	,	3		;control-c
EQU		CNTRLE	,	$05		;control-e
EQU		BS	,	$08		;backspace
EQU		TAB	,	$09		;tab
EQU		LF	,	$0A		;line feed
EQU		FF	,	$0C		;form feed
EQU		CR	,	$0D		;carriage return
EQU		CNTRLP	,	$10		;control-p
EQU		CNTRLR	,	$12		;control-r
EQU		CNTRLS	,	$13		;control-s
EQU		CNTRLU	,	$15		;control-u
EQU		CNTRLX	,	$18		;control-x
EQU		CNTRLZ	,	$1A		;control-z (end-of-file mark)
EQU		DEL	,	$7F		;rubout
;
;   Set origin for CP/M
;
...
; flag is set on a return of $FF.
;
ENTRY1:	CALL	ENTRY
	LD	(RTNCODE),A	;save return code.
	INC	A		;set zero if $FF returned.
	RET	
;
...
;
EQU NUMCMDS ,	6		;number of commands
CMDTBL:	DEFB	'DIR '
	DEFB	'ERA '
	DEFB	'TYPE'
	DEFB	'SAVE'
...
;
HALT:	LD	HL,$76F3	;'DI HLT' instructions.
	LD	(CBASE),HL
	LD	HL,CBASE
	JP	(HL)
;
...
UNKWN4:	POP	HL
	...
	LD	(FCB+32),A
	LD	DE,TFCB		;move it into place at(0$5C).
	LD	HL,FCB
	LD	B,33
	CALL	HL2DE
	LD	HL,INBUFF+2	;now move the remainder of the input
UNKWN5:	LD	A,(HL)		;line down to (00$80). Look for a non blank.
	OR	A		;or a null.
	JP	Z,UNKWN6
	CP	' '
	JP	Z,UNKWN6
...
EQU CCPSTACK ,	^	;end of ccp stack area.
;
...
; (PATTRN1) or cp/m will HALT. Why?  - WHY, I think it's because it makes sure that the CCP is exactly 800 bytes as assembled. 
; Typically this goes straight onto the BDOS, so ensures the BDOS and CCP are of the same version. 
;
